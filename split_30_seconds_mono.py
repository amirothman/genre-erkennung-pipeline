...
def ffmpeg_process(filepath,format_ending,cmd):
    filepath = filepath.replace("./","")
    filepath = filepath.replace(".{0}".format(format_ending),"")
    commandos = []
    split_commando = cmd.format(filepath,format_ending)
    commandos.append(split_commando)
...
def thirty_seconds(filepath,format_ending):
    ...
    filepath = filepath.replace(".{0}".format(format_ending),"")
    commandos = []
    split_commando = "ffmpeg -i {0}.{1} -f segment -segment_time 30 -c copy {0}-%03d.{1}".format(filepath,format_ending)
    commandos.append(split_commando)
    del_commando = "rm {0}".format("".join([filepath,".",format_ending]))
    commandos.append(del_commando)
...
def to_mono(filepath,format_ending):
    ...
    filepath = filepath.replace(".{0}".format(format_ending),"")
    commandos = []
    mono_commando = "ffmpeg -i {0}.{1} -ac 1 {0}-mono.{1}".format(filepath,format_ending)
    commandos.append(mono_commando)
    del_commando = "rm {0}".format("".join([filepath,".",format_ending]))
    commandos.append(del_commando)
    for cmd in commandos:
        print(cmd)
